...
FROM
    OPENROWSET(
        BULK 'https://loskisynapsedatalake.dfs.core.windows.net/loskisynapsefilesystem/census/PUMS/1-year/2017/Population/psam_pusa.csv',
        FORMAT = 'CSV',
        PARSER_VERSION = '2.0'
    ) AS [result];
...
SELECT  
 ...
 FROM
    OPENROWSET(
        BULK 'https://loskisynapsedatalake.dfs.core.windows.net/loskisynapsefilesystem/census/PUMS/1-year/2017/Population/*.csv',
        FORMAT = 'CSV',
        PARSER_VERSION = '2.0'
        , FIRSTROW = 2
    ) 
...
CREATE VIEW PopulationHealthInsurance AS
SELECT  
    *
    , result.filepath(1) as FileName
 FROM
...
INNER JOIN dbo.HispanicRecode as Hr
ON h.HispanicDetail = Hr.Code
group by r.Region
  , Hr.HispanicDetail
  , HealthInsuranceEmployer
  , HealthInsuranceDirect
