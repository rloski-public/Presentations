...
IF NOT EXISTS (SELECT * FROM sys.external_data_sources WHERE name = '{StorageContainer}_{StorageAccount}_dfs_core_windows_net') 
	CREATE EXTERNAL DATA SOURCE [{StorageContainer}_{StorageAccount}_dfs_core_windows_net] 
	WITH (
		LOCATION = 'abfss://{StorageContainer}@{StorageAccount}.dfs.core.windows.net'
	)
GO
...
CREATE EXTERNAL TABLE dbo.DivisionCode (
	...
	DATA_SOURCE = [{StorageContainer}_{StorageAccount}_dfs_core_windows_net],
